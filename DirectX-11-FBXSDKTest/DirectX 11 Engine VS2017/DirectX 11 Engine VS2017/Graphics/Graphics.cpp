...
bool Graphics::Initialize(HWND hwnd, int width, int height)
{
	...
	bool b1 = model->GetFBXModel()->GetMeshData()->kVertexType.IsFlagExist(FBXElement_Position);
	bool b2 = model->GetFBXModel()->GetMeshData()->kVertexType.IsFlagExist(FBXElement_Normal);
	bool b3 = model->GetFBXModel()->GetMeshData()->kVertexType.IsFlagExist(FBXElement_Tangent);
	bool b4 = model->GetFBXModel()->GetMeshData()->kVertexType.IsFlagExist(FBXElement_Color);
	bool b5 = model->GetFBXModel()->GetMeshData()->kVertexType.IsFlagExist(FBXElement_UV1);
	bool b6 = model->GetFBXModel()->GetMeshData()->kVertexType.IsFlagExist(FBXElement_UV2);
	return true;
}
...
void Graphics::RenderFrame(float dt)
{
	...
	spriteBatch->Begin();
	auto text = "cam rot: " + std::to_string(camera.GetRotationFloat3().x) + " "
							+ std::to_string(camera.GetRotationFloat3().y) + " "
							+ std::to_string(camera.GetRotationFloat3().z);
...
bool Graphics::InitializeScene()
{
	try
	{
		...
		camera.SetPosition(-60.0f, 0.0f, 1000.0f);
